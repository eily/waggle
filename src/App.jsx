...
function getQueenMilestones(queen) {
  if (!queen.qc_capped_date) return [];
  const capped = queen.qc_capped_date;
  return [
    { label:"QC capped", date:capped, confirmed:true },
    { label:"Queen expected to emerge", date:addDays(capped,16), confirmed:!!queen.queen_emerged_date, actual:queen.queen_emerged_date },
    ...
    { label:"Earliest expected eggs", date:addDays(capped,25), confirmed:!!queen.eggs_first_seen, actual:queen.eggs_first_seen },
  ];
}
...
function generateActions(hive, inspections, queens) {
  ...
  if (last) {
    if (last.qc_action === "1 left" || last.qc_action === "2 left")
      actions.push({ text:`Check if queen cell has hatched — left ${last.qc_action} on ${formatDate(last.visit_date)}`, priority:"high" });
    if (last.brood_status === "No eggs")
      actions.push({ text:`No eggs seen ${formatDate(last.visit_date)} — check for new queen or eggs`, priority:"high" });
    if (last.varroa === "High")
    ...
    if (last.supers_total > 0)
      actions.push({ text:`Check super fill — ${last.supers_total} super${last.supers_total>1?"s":""} on hive`, priority:"medium" });
    if (days > 10)
      actions.push({ text:`Inspection overdue — last visit ${formatDate(last.visit_date)} (${days} days ago)`, priority:"low" });
  } else {
...
function SegPicker({ options, value, onChange, small }) {
  return (
    <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
      {options.map(opt => {
        const sel = opt === value;
        return <button key={opt} onClick={() => onChange(opt)} style={{ flex:1, minWidth:52, padding:small?"9px 4px":"13px 4px", border:sel?`1.5px solid ${TEAL}`:"0.5px solid #D1D5DB", borderRadius:10, background:sel?TEAL:"#fff", color:sel?"#fff":"#374151", fontSize:small?12:14, fontWeight:sel?700:400, cursor:"pointer", fontFamily:"inherit", lineHeight:1.2 }}>{opt}</button>;
      })}
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display:"flex", gap:6, marginBottom:8 }}>
        {[1,2,3,4,5].map(n => <button key={n} onClick={() => onChange(n)} style={{ flex:1, padding:"14px 0", border:n===value?`1.5px solid ${TEAL}`:"0.5px solid #D1D5DB", borderRadius:10, background:n===value?TEAL:"#fff", color:n===value?"#fff":"#374151", fontSize:16, fontWeight:n===value?700:400, cursor:"pointer", fontFamily:"inherit" }}>{n}</button>)}
      </div>
...
function FormRow({ label, sublabel, children, last }) {
  return (
    <div style={{ padding:"14px 16px", borderBottom:last?"none":"0.5px solid #F3F4F6" }}>
      {label && <div style={{ marginBottom:10 }}><span style={{ fontSize:14, fontWeight:600, color:"#111827" }}>{label}</span>{sublabel&&<span style={{ fontSize:12, color:"#9CA3AF", marginLeft:6 }}>{sublabel}</span>}</div>}
      {children}
    </div>
  );
}
...
function LoadingScreen() {
  return (
    <div style={{ height:"100%", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:TEAL }}>
      <div style={{ fontSize:32, fontWeight:800, color:"#fff", marginBottom:12 }}>Waggle</div>
      <div style={{ fontSize:14, color:"#9FE1CB" }}>Loading your hives…</div>
    </div>
  );
}
      ...
      <div style={{ background:"#fff", borderRadius:14, border:"0.5px solid #E5E7EB", marginBottom:10, overflow:"hidden" }}>
        <div onClick={() => setOpen(!open)} style={{ padding:"14px 16px", display:"flex", alignItems:"center", gap:12, cursor:"pointer", borderBottom:open?"0.5px solid #F3F4F6":"none" }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:12, color:"#9CA3AF", marginTop:1 }}>{hive.apiary_name} · {last?`Last inspected ${formatDate(last.visit_date)}`:"Never inspected"}</div>
          </div>
...
function InspectionForm({ hive, queens, onSave, onBack }) {
  ...
  const [form, setForm] = useState({
    ...
    brood_status:"BIAS", brood_frames:3,
    stores:5, room:4,
    health:"OK", health_note:"",
    varroa:"Not checked", temperament:null,
    feed_given:"None", feed_qty:"",
    supers_change:0,
  ...
  const supersTotal = Math.max(0, hive.super_count + form.supers_change);
  ...
  return (
    <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
      <div style={{ background:TEAL, padding:"max(env(safe-area-inset-top,0px),12px) 16px 14px", flexShrink:0 }}>
        ...
        <div style={{ fontSize:12, color:"#9FE1CB", marginTop:2 }}>{hive.apiary_name} apiary · {form.visit_date} · {form.visit_time}</div>
      </div>
      <div style={{ flex:1, overflowY:"auto", background:GRAY, padding:"12px", WebkitOverflowScrolling:"touch" }}>
        ...
        <FormSection title="Brood">
          ...
          <FormRow label="Stores" sublabel="space available in brood box"><Slider min={0} max={20} step={1} value={form.stores} onChange={v=>set("stores",v)} unit=" fr" /></FormRow>
        ...
        <FormSection title="Hive configuration">
          <FormRow last>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              {[["qe","QE fitted"],["crown_board","Crown board"],["porter_escapes","Porter escapes"],["entrance_reducer","Entrance reducer"],["mouse_guard","Mouse guard"],["insulation","Insulation"]].map(([k,l]) => (
                ...
                {milestones.map((m,i) => (
                  <div key={i} style={{ display:"flex", gap:10, padding:"7px 0", position:"relative" }}>
                    ...
                    <div>
                      ...
                      <span style={{ fontSize:11, background:m.confirmed?TEAL_LIGHT:AMBER_LIGHT, color:m.confirmed?"#085041":"#92400E", padding:"2px 8px", borderRadius:99, fontWeight:700, display:"inline-block", marginTop:3 }}>{m.confirmed?"Confirmed":"Projected"}</span>
                    </div>
                  </div>
                ))}
              </div>
  ...
  return (
    <div style={{ height:"100%", display:"flex", flexDirection:"column", background:TEAL }}>
      <div style={{ padding:"max(env(safe-area-inset-top,0px),12px) 16px 14px", flexShrink:0 }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          {Object.keys(APIARIES).map(name => <button key={name} onClick={() => setApiary(name)} style={{ padding:"8px 18px", borderRadius:99, background:apiary===name?"#fff":"rgba(255,255,255,0.15)", color:apiary===name?TEAL:"#fff", border:"none", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>{name}</button>)}
        </div>
      </div>
...
export default function App() {
  ...
  const handleSaveInspection = async (inspection, supersChange) => {
